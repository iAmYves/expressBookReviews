...
public_users.post("/register", (req, res) => {
  ...
  if (username && password) {
    if (!doesExist(username)) {
      ...
      return res
        .status(200)
        .json({ message: "You've successfully registered. You can now login" });
    } else {
...
public_users.get("/isbn/:isbn", function (req, res) {
  ...
  fetchBooks()
    .then((books) => {
      ...
      const searchIsbn = booksList.filter((book) => book.isbn === isbn);
      if (searchIsbn.length > 0) {
        return res.status(200).send("Book Name: " + JSON.stringify(searchIsbn[0].title));
      } else {
